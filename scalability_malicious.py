...
Fixes:
✅ Numeric-only feature preprocessing (avoids 'could not convert string to float')
✅ Dynamic sharding with reputation-based reconfiguration
✅ Robust ML-based malicious detection
✅ Parallel shard-level throughput and latency tracking
"""
...
print(f"✅ ML models trained on {len(miner_df)} samples | Malicious nodes: {sum(y_real)}")
...
def simulate_run(mode="ml_reputation", seed=0, malicious_frac=0.2):
    ...
    for b in range(BLOCKS_PER_RUN):
        ...
        # Simulated block latency — lower for higher shard counts
        ...
        if mode == "ml_reputation" and ML_DETECTION and (b + 1) % EPOCH_BLOCKS == 0:
            ...
            print(f"♻️ Epoch {(b+1)//EPOCH_BLOCKS}: Reconfiguring shards...")
            ...
            print("⭐ New shard leaders:", shard_leaders)
...
def run_batch():
    ...
    print("\n✅ Simulation completed! Results saved in:", OUTDIR)
